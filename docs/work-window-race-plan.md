...
- Work windows now have a `runtimeKind`: `token_plan` keeps the native Arena tool loop, while `codex_cli` invokes local Codex CLI in the window worktree. Codex CLI state is isolated under `.codex-runtime`; Token Plan provider settings are passed per invocation with `-c`, and the API key is passed only through `ALIYUN_TOKEN_PLAN_API_KEY`.
